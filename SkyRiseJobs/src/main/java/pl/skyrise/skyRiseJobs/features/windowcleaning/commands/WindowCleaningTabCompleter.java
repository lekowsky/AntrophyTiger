...
public class WindowCleaningTabCompleter implements TabCompleter {
    ...
    public WindowCleaningTabCompleter(WindowCleaningJobModule plugin) {
        this.plugin = plugin;
    }

    @Override
    public List<String> onTabComplete(CommandSender sender, Command cmd, String alias, String[] args) {
        if (!sender.hasPermission("skyrisejobs.windowcleaning.admin")) return Collections.emptyList();
        ...
        if (args.length == 3 && args[0].equalsIgnoreCase("setskill")) {
            return plugin.getSkills().stream()
                    .map(pl.skyrise.skyRiseJobs.api.job.JobSkill::getId)
